...
describe("Atlas Improvements", () => {
  describe("User Atlas Prompt", () => {
    it("should include instructions for fact vs assumption distinction", () => {
      ...
      expect(userAtlasContent).toContain("Only store information that the user has explicitly stated or confirmed as fact");
      expect(userAtlasContent).toContain("Do not store assistant assumptions, guesses, or interpretations");
      expect(userAtlasContent).toContain("MANDATORY:** Include specific dates (YYYY-MM-DD format)");
      expect(userAtlasContent).toContain("immediately remove or update** the contradicted information");
      expect(userAtlasContent).toContain("Actively remove information that is clearly outdated");
    });
  });
  ...
  describe("Assistant Atlas Prompt", () => {
    it("should include instructions for observations vs assumptions", () => {
      ...
      // Check that key improvements are present in the prompt
      expect(assistantAtlasContent).toContain("Record only what can be directly observed from interactions");
      expect(assistantAtlasContent).toContain("Avoid storing assumptions, guesses, or interpretations as established facts");
      expect(assistantAtlasContent).toContain("MANDATORY:** Include the date (YYYY-MM-DD format)");
      expect(assistantAtlasContent).toContain("Actively remove outdated relationship patterns");
      expect(assistantAtlasContent).toContain("Date Tracking:** When adding or updating any entry");
    });
  });
